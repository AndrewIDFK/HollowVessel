...
namespace HollowVessel.Projectiles.Nails.ChannelledNail
{
    public class ChannelledNail : ModProjectile
    {
               ...
               /*if (projectile.ai[0] == 40)
                ...
                if (projectile.ai[0] > 120)
                {
                   /* if (projectile.ai[0] % 80 == 40)
                    ...
                    int dust2 = Dust.NewDust(new Vector2(player.Center.X - 4, player.Center.Y + player.height / 2 * player.gravDir), 1, 1, 263, 5, -3 * player.gravDir, 0, default(Color), 1);
                    Main.dust[dust2].noGravity = true;
                    int dust3 = Dust.NewDust(new Vector2(player.Center.X - 4, player.Center.Y + player.height / 2 * player.gravDir), 1, 1, 263, -5, -3 * player.gravDir, 0, default(Color), 1);
        ...
        public override void Kill(int timeLeft)
        {
			...
			else if(projectile.ai[0] > 120)
            {
                ...
                Projectile.NewProjectile(pos.X, pos.Y, projectile.velocity.X * 2, projectile.velocity.Y * 2, mod.ProjectileType("ChannelledNailSlash"), projectile.damage * 10, projectile.knockBack * 6, projectile.owner);
            }
        }
    }
}